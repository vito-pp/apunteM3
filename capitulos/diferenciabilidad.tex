...
\begin{definition}
  ...
  Y a su vez, en el caso particular de que $n=m$; se define el \emph{jacobiano de $\mathbf{F}$ en $\mathbf{x}_0$}, y se lo nota por $J_{\mathbf{F}}(\mathbf{x}_0)$, al determinante de la matriz diferencial, \emph{i.e.}
  \[
...
\mbox{}
  ...
  Sean $f, g$ funciones 
    ...
    \end{align*}
  y un punto $\mathbf{x}_0 \in \interior{A} \text{ tal que } g(\mathbf{x}_0) \in \interior{B}$. Supongamos que $g$ es diferenciable en $\mathbf{x}_0$ y que $f$ es diferenciable en $yo = g(\mathbf{x}_0)$. Entonces la composic\'on $f \circ g$ es diferenciable en $\mathbf{x}_0$ y, adem\'as,
  \[
   \boldsymbol{D}(f \circ g)_{(\mathbf{x}_0)} = \boldsymbol{D}f_{(y_0)} 
   \boldsymbol{D}g_{(\mathbf{x}_0)}.
  \]
\end{theorem}
