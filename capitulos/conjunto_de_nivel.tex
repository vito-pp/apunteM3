...
\begin{definition} \textbf{Conjunto de nivel.} 
    ...
    Sea $f: U \subset \mathbb{R}^n \rightarrow \mathbb{R}$ y sea $c \in \mathbb{R}$, se define  \emph{el conjunto de nivel de $f$ de nivel}  $c$, y se nota $C(c,f)$,  a  
    \[
    C(c,f)=\{ \mathbf{x} \in U \;|\; f (\mathbf{x})=c \}.
     \]
    Es decir,  $C(c,f)$ son aquellos puntos $\vect{x}\in\ U$ para las cuales $f(x)=c$, o dicho de otra manera,  $C(c,f)$ es la preimagen de $c$ por $f$.   Si $n=2$ hablamos de curva de nivel  y si $n=3$  hablamos de superficie de nivel.  
...
\begin{definition}
        Se llama \emph{corte transversal} de un conjunto \( S \subseteq \mathbb{R}^n \) a la intersección de \( S \) con un hiperplano \( H \subseteq \mathbb{R}^n \).
...
\begin{example}\label{example:ex1graf}   Sea  el campo  $f: \mathbb{R}^{2} \rightarrow \mathbb{R} \:|\:  f(x,y)=x^2+y^2.$

    1. Calcular y graficar los conjuntos de nivel de $f$.
    
    2. Hacer un gr\'afico de  $\text{Graf}(f)$.
    
    
    
    Observemos que, dado $c \in \mathbb{R}$: 
     $$C(c,f)=\{(x,y) \in U \;|\;   x^2+y^2  =c \}.$$
    Tomando, para ilustrar,  $c=0$, $c=1$ y $c=4$, 
    % quedando (ver (\ref{circ})) 
     \[
    C(0,f)=\{(x,y) \in U \;|\; x^2+y^2=0 \}
    \]
     \[
...
\begin{example} \label{example:ej2graf}  Sea  el campo  $g: \mathbb{R}^{2} \rightarrow \mathbb{R} \;|\; g(x,y) = \sqrt{x^2+y^2}.$
  \begin{enumerate}
    \item Calcular y graficar los conjuntos de nivel de $g$. 
    \item Hacer un gr\'afico de  $\text{Graf}(g)$.
  \end{enumerate}
Observemos que, dado $c \in \mathbb{R}$, 
     $$C(c,f)=\{(x,y) \in U \;|\;   \sqrt{x^2+y^2}  =c \}.$$
    Tomando, para ilustrar,  $c=0$, $c=1$ y $c=2$, 
    % quedando (ver (\ref{circ})) 
    \[
        C(0,f)=\{(x,y) \in U \;|\; \sqrt{x^2+y^2}=0 \}
        \]
         \[
        C(1,f)=\{(x,y) \in U \;|\; \sqrt{x^2+y^2}=1 \}
        \]
         \[
        C(2,f)=\{(x,y) \in U \;|\; \sqrt{x^2+y^2}=2 \}.
         \]
...
\begin{center}
    ...
    \end{tikzpicture}
\end{center}
En el \autoref{example:ej2graf}, tomando el corte en $x=0$ podemos observar que:
\(
f(0,y)=\sqrt{0^2+y^2} \implies f(y)=|y|
\)
\begin{center}
    \begin{tikzpicture}
        % Ejes coordenados
