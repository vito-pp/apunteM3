\begin{definition}
    Sea $f:U\subset\Rn{n}\to\R$ con $U$ abierto y $f$ diferenciable en $U$.   Se define el \emph{gradiente} de $f$ evaluado en $\mathbf{x}\in U$, y se nota $\grad f(\mathbf{x})$,  como el vector en el espacio $\Rn{n}$ dado por 
    \[
...
\begin{definition}
    Sea $\mathbf{F}:U\subset\Rn{n}\to\Rn{n}$ con  $U$ abierto y $\mathbf{F}$ de clase $\mathcal{C}^1(U)$. Se define la \emph{divergencia} de $\mathbf{F}$ como
    \[
...
\begin{definition}
    Sea $\mathbf{F}:U\subset\Rn{3}\to\Rn{3}$ con $U$ abierto y $\mathbf{F}$ de clase $\mathcal{C}^1(U)$. Se define el \emph{rotor} de $\mathbf{F}$  evaluado en $\mathbf{x}\in U$, y se nota  $\grad\times\mathbf{F}(\mathbf{x})$,   como el vector en el espacio $\Rn{3}$ dado por 
    \[
    ...
    Sea $\mathbf{F}:U\subset\Rn{2}\to\Rn{2}$   con $U$ abierto y $\mathbf{F}$ de clase $\mathcal{C}^1(U)$. Se define el \emph{rotor} de $\mathbf{F}$  evaluado en $\mathbf{x}\in U$, y se nota  $\grad\times\mathbf{F}(\mathbf{x})$,   como el escalar dado por
    \[
...
\begin{definition}
    Sea $f:U\subset\Rn{n}\to\R$ con  $U$ abierto y $f$ de clase $\mathcal{C}^2(U)$.  Se define el \emph{laplaciano} de $f$, y se lo nota $\Delta f$ o $\grad^2 f$, a
    \[
        \Delta f=\grad^2f=\grad\cdot\grad f.  
    \]
    Y para el caso de un campo vectorial $\mathbf{F}:U\subset\Rn{n}\to\Rn{n}$ de clase $\mathcal{C}^2(\Rn{n})$, el \emph{laplaciano} escalar
    \[
...
\begin{definition}
    Sea un campo vectorial $\mathbf{F}:U\subseteq\Rn{n}\to\Rn{n}$.  Si $\grad\times\mathbf{F}=\mathbf{0}$ se llama  \emph{irrotacional}.
  \end{definition}  
  
  
  \begin{definition} Sea un campo vectorial $\mathbf{F}:U\subseteq\Rn{n}\to\Rn{n}$.
     Si $\grad\cdot\mathbf{F}=0$  se llama  \emph{solenoidal}.
\end{definition}
\begin{definition}
    Sea un campo escalar $f$ o un campo vectorial $\mathbf{F}$. Si $\Delta f=0$ o $\Delta \mathbf{F}=0$  se llama  \emph{arm\'onico}.
\end{definition}
...
\begin{propertie}
    ...
    \end{enumerate}
\end{propertie}
\begin{propertie}\textbf{Productos.} Sea $f,g$ de clase $\mathcal{C}^1$ y $\mathbf{F},\mathbf{G}$ de clase $\mathcal{C}^1$
    \begin{enumerate}
