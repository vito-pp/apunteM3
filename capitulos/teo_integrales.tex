...
\begin{obs}\textbf{Regla de la mano derecha.}
    ...
    orientaci\'ones ``hacia arriba o hacia abajo''. Entonces se define una convenci\'on para la orientaci\'on de este tercer eje de manera que 
    \[
        \vu{i}\times\vu{j}=\vu{k},
    \]  
    donde $\vu{i}$, $\vu{j}$ y $\vu{k}$ son los versores c\'anonicos para los ejes cartesianos $x,\;y,\;z$, respectivamente. Luego \emph{la regla de la mano derecha} es una mnemot\'ecnica que sirve para recordar dicha orientaci\'on.
    \input{figs/mano_derecha.tex}
\end{obs}

\begin{theorem}
    \textbf{Teorema de Green}. Sea $D\subset \Rn{2}$ un conjunto simplemente conexo (\autoref{def:conjuntosimpconexo}) y sea la curva simple y cerrada  $\partial D$ su frontera. Si  $\mathbf{F}:D\to\Rn{2}$ es continuo y de clase $\mathcal{C}^1$, entonces
    \[
        \oint_{\partial D^{+}} \mathbf{F}\cdot d\mathbf{s} = \iint_D \grad\times\mathbf{F}\cdot d\mathbf{A}.
    \]
\end{theorem}

\begin{obs} Sea $\mathbf{F}:D\subset \Rn{2}\to\Rn{2}$, con $\mathbf{F}(x,y)=(P(x,y),Q(x,y))=P(x,y)\vu{i}+Q(x,y)\vu{j}$. Entonces, si se cumple el teorema de Green para $\mathbf{F}$ en $D$, se puede escribir el teorema de la siguiente manera alternativa:
\[
...
y 
\[
    \left(\dif{Q}{x}-\dif{P}{y}\right)\vu{k}=\nabla\times\mathbf{F}.
\]
\textcolor{red}{no se bien como escribir esto, pq $\vu{k}$ es una direcci\'on ``virtual'' en R2.}
\end{obs}
...
\begin{theorem}
\textbf{Teorema de la divergencia o de Gauss}. Sea $S\subset\Rn{3}$ una superficie que encierra un volumen $\Omega$, \textit{i.e.} $S=\partial \Omega$, orientada de manera exterior.  Si $\mathbf{F}:A \subset \Rn{3} \to\Rn{3}$ es  un campo vectorial continuo y de clase $\mathcal{C}^1(\interior{A})$ tal que $\Omega \subset \interior{A}$, entonces
\[
