...
\begin{obs} Para resolver límites por definici\'on usualmente se utilizan ciertas cotas conocidas. En primer lugar, fijamos $(x,y)\neq (0,0)$.
  \begin{enumerate}
  \item 
  \[
...
\[
\forall \varepsilon > 0\; \exists\:\delta > 0 \;|\; |f(x,y) - L| < \varepsilon  \implies \forall (x,y) \in \R^2 \;|\;0 < \|(x-x_0,y-y_0)\| < \delta
\]
Entonces,
\[
...
\begin{enumerate}
  \item
Se escribir\'a la definici\'on con el límite solicitado, proponiendo $L=0$.
\[
\forall \varepsilon > 0 \; \exists\: \:\delta > 0 \;|\; \left| \frac{x^2y}{x^2+y^2} - 0\right| < \varepsilon  \implies \forall (x,y) \in \R^2 \;|\; 0 < \|(x-0,y-0)\| < \delta
\]
...
Se escribir\'a la definici\'on con el límite solicitado, proponiendo $L=0$.
\[
\forall \varepsilon > 0 \;\exists\: \:\delta > 0 \;|\; \left| \frac{x^2sen(y)}{x^2+y^2} - 0\right| < \varepsilon \implies \forall (x,y) \in \R^2 \;|\; 0 < \|(x-0,y-0)\| < \delta.
\]
Partimos de
\[
...
Basta con tomar $\delta=\epsilon$. Entonces
\[
\frac{x^2sen (y)}{x^2+y^2}\leq\sqrt{x^2+y^2}<\delta=\varepsilon.
\]
\end{enumerate}
...
\begin{example} Calcular los siguientes l\'imites usando \'algebra de l\'imites.
 ...
 \begin{enumerate}
     ...
     \[
         \lim_{(x, y) \to (x_0, y_0)} \frac{e^{x^2} \log(\cos y)}{x^2 + (y + 1)^2} =  \frac{e^0\log 1}{0 + 1^2} = \frac{1 \cdot 0}{1} = 0.
     \]
 \end{enumerate}
...
\begin{corollary} \textbf{L\'imites por curvas.} \label{cor:lim_curva}
  ...
  \]
  y, adem\'as, $\left( g_1(t), g_2(t) \right) \in A \, \forall t \in \left( I \smallsetminus \{t_0\} \right),$ se cumple que
  \[
