\begin{definition}\label{def:a-trozos}
    A un campo continuo $\boldsymbol{\sigma}:[a,b]\subseteq\R\to\Rn{n}$ se lo llama $\mathcal{C}^1$ \emph{a trozos} si existe un conjunto $\{t_i\}_{i=0}^n$ partici\'on de $[a,b]$ tal que $\boldsymbol{\sigma}$ es de clase $\mathcal{C^1}$ en $I_i=(t_i, t_{i+1})$.
\end{definition}

\begin{definition} A un campo $\boldsymbol{\sigma}:I\subseteq\R\to\Rn{n}$ continua y de clase $\mathcal{C}^1$ a trozos se lo llama \textit{trayectoria}. 
\end{definition}
