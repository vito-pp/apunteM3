...
\begin{example}
    ...
    Adem\'as,   $\text{Dom}(f)=  [0,+\infty)$,   $\text{Cod}(f)=  \mathbb{R}$ y $\text{Img}(f)= [-1,+\infty)$ .
...
\begin{example}  Sea $$f: [-2,2]  \subseteq \mathbb{R} \rightarrow  \mathbb{R} \:|\:  f(x)=x^2$$ 
 ...
 Se arma la siguiente tabla de valores, para valores discretos de $x\in [0,2]$:
    ...
    $\text{Dom}(f)=  [-2,2]$,   $\text{Cod}(f)=  \mathbb{R}$ y $\text{Img}(f)= [0,4]$
...
\begin{definition}{\textbf{Polinomio de Taylor.}}
    Sea $f$ una función con derivadas de orden $n$ en $a \in \text{Dom}(f)$. Se llama \textit{polinomio de Taylor de orden $n$ centrado en $a$ de la función $f$}, y se nota por $T_n[f,a](x)$, a:
    \begin{equation*}
...
\begin{definition}{\textbf{Resto de Taylor.}}
    ...
    \begin{equation*}
        R_n[f,a](x) = f^{(n+1)}(c)\frac{(x-x_0)^{n+1}}{(n+1)!}.
    \end{equation*}
    A esta última se la llama \textit{fórmula de Lagrange para resto}.
