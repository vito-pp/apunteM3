\textcolor{red}{AGREGAR DEFINICION PARTICION}
...
\begin{definition} {\textbf{Imagen.}}
    Dada una funci\'on $f: A \subseteq \mathbb{R} \rightarrow B \subseteq \mathbb{R},$  se llama \textit{imagen} de una función y se nota  $\text{Im}(f)$, a
    al conjunto 
    \begin{equation*}
...
\begin{definition}{\textbf{Polinomio de Taylor.}}
    Sea $f$ una función con derivadas de orden $n$ en $x=a \in \text{Dom}(f)$. Se llama \textit{Polinomio de Taylor} 
    de órden $n$ de la función $f$ en $x=a$, y se simboliza $T_{f,a,n}(x)$, a:
    \begin{equation*}
...
\begin{definition}{\textbf{Resto de Taylor.}}
    Se llama \textit{Resto de Taylor}, o término de
    error de la aproximación, a:
    \begin{equation*}
        R_{f,a,n}(x) = f(x)-T_{f,a,n}(x)
    \end{equation*}
    Sea $f$ una función con derivadas de orden $n+1$ en el intervalo cerrado entre $a$ y $x$. 
    Existe $c\in\mathbb{R}$ en el intervalo abierto entre $a$ y $x$ tal que:
    \begin{equation*}
        R_{f,a,n}(x) = f^{(n+1)}(c) \cdot \frac{(x-x_0)^{n+1}}{(n+1)!}
    \end{equation*}
    A esta última se la llama la \textit{fórmula de Lagrange del Resto}.
