...
\begin{theorem} \textbf{Polinimio de Taylor de primer orden.}
  ...
  Sea $f:U\subseteq\Rn{n}\to\R$ de clase $\mathcal{C}^1$. Se define el \emph{polinomio de Taylor de primer orden centrado en $\mathbf{x}_0$ de $f$}, y se nota por $T_1[f,\mathbf{x}_0](\mathbf{x})$, a
  \[
...
\begin{theorem} \textbf{Polinimio de Taylor de segundo orden.}
  ...
  Sea $f:U\subseteq\Rn{n}\to\R$ de clase $\mathcal{C}^2$. Se define el \emph{polinomio de Taylor de segundo orden centrado en $\mathbf{x}_0$ de $f$}, y se nota por $T_2[f,\mathbf{x}_0](\mathbf{x})$, a
  \[
...
\begin{obs}
  Notar que es indistinto evaluar el signo de $f_{xx}$ o $f_{yy}$; ya que si $f:A\subset\Rn{2}\to\R$ y $f\in\mathcal{C}^2$, entonces
  \[
    \boldsymbol{H}_f=\begin{bmatrix}
                      f_{xx} & f_{xy}\\
                      f_{yx} & f_{yy}
                     \end{bmatrix}\implies \text{det}(\boldsymbol{H}_f)=f_{xx}f_{yy}-f_{xy}^2,
  \]
  $\because f_{xy}=f_{yx}$, por el \autoref{teo:schwarz}. Luego si 
  \[
