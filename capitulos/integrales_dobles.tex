...
\begin{obs}
\textcolor{red}{agregar condiciones sobre f para la  existencia del limite. que sea continua, salvo discontinuidad en curvas.}
\end{obs}
...
\begin{definition}\label{int_D} \textcolor{red}{Hacer un dibujo. como en el tromba}
Dada $f:D\to\R$, se define la funci\'on $f^*$ tal que \textcolor{red}{quien es D?}
\[
    f^*(x,y)=
    \begin{dcases*}
        f(x,y) & si $(x,y)\in D$ \\[.2cm]
        0        & si $(x,y)\notin R\smallsetminus D.$
    \end{dcases*}
\]
\textcolor{red}{quien es R?}
...
\begin{definition}
    Sea $\mathbf{F}:A\subseteq\Rn{n}\to\Rn{n}$ un campo $\mathcal{C}^1(A)$ y sea $\boldsymbol{D}_\mathbf{F}$ \textcolor{red}{agregar ref de Df} su matriz diferencial definida en $A$. Se define el \textbf{jacobiano}, o \textbf{determinante jacobiano}, y se lo nota $J_\mathbf{F}$ a 
    \[
...
\begin{example} \textbf{Coordenadas polares (\(n = 2\))} \newline
    Sea \( F(r, \theta) = (x,y)= (r\cos\theta, r\sin\theta) \). Calculamos la matriz jacobiana:
   ...
   \begin{example} \textbf{Coordenadas cilíndricas (\(n = 3\))} \newline
       Sea \( F(r, \theta, z) = (x, y, z) = (r\cos\theta, r\sin\theta, z) \). Calculamos la matriz jacobiana:
   ...
   \begin{example} \textbf{Coordenadas esféricas (\(n = 3\))} \newline
       Sea \( F(\rho, \theta, \phi) = (x, y, z) = (\rho\sin\phi\cos\theta, \rho\sin\phi\sin\theta, \rho\cos\phi) \). Calculamos la matriz jacobiana:
