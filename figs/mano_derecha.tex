...
\begin{tikzpicture}
    ...
    \draw[thick vector, myred]
      (O) --++ (148:3.3) coordinate (X) node[left,scale=1.7,yshift=3pt] {$\vect{i}$};
    \draw[thick vector, myblue]
      (O) --++ (-172:3.25) coordinate (Y) node[left,scale=1.7,yshift=3pt] {$\vect{j}$};
    \draw[thick vector, mypurple]
      (O) --++ (82:3.2) node[above=0,scale=1.7] {$\vect{k}$};
